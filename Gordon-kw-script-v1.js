...
function checkRankingsOnly() {
  ...
  const batchSize = 20; // Optimized for large datasets
  const totalBatches = Math.ceil(totalKeywords / batchSize);
  
  SpreadsheetApp.getUi().alert(
    '🚀 Starting Ranking Check', 
    `Processing ${totalKeywords} keywords in ${totalBatches} batches of ${batchSize}.\n\nThis will take approximately ${totalBatches * 1.5} minutes.`, 
  ...
  console.log(`📦 Processing ${totalKeywords} keywords in ${totalBatches} batches`);
    ...
    for (let batchIndex = 0; batchIndex < totalBatches; batchIndex++) {
      ...
      // Submit all keywords in this batch
      const taskIds = [];
      for (let i = 0; i < batchKeywords.length; i++) {
        const item = batchKeywords[i];
        const actualRowNumber = startIndex + i + 2;
        processed++;
      
      try {
        console.log(`📤 Submitting ${i + 1}/${batchKeywords.length}: "${item.keyword}" (Row ${actualRowNumber})`);
        console.log(`📤 Submitting ranking job for: "${item.keyword}" in Location`);
        
        const taskId = submitKeywordRankingJob(item.keyword, item.lat, item.lng);
        if (taskId) {
          taskIds.push({ taskId: taskId, row: actualRowNumber, keyword: item.keyword });
          console.log(`✅ Submitted: ${taskId}`);
        } else {
          console.log(`❌ Failed to submit: ${item.keyword}`);
          failed++;
    ...
    if (taskIds.length > 0) {
      console.log(`⏳ Waiting 1.5 minutes for DataForSEO to process batch ${batchIndex + 1}...`);
      
      // Show progress in sheet
      sheet.getRange(1, 12).setValue(`⏳ Processing Batch ${batchIndex + 1}/${totalBatches}...`); // Column L
      
      Utilities.sleep(90000); // Wait 1.5 minutes (90 seconds)
      ...
      for (const taskData of taskIds) {
        try {
          ...
          if (rankingResults && rankingResults.rankings.length > 0) {
            ...
            console.log(`⚠️ No ranking found for: ${taskData.keyword}`);
      ...
      sheet.getRange(1, 12).setValue(`✅ Completed batch ${batchIndex + 1}/${totalBatches} - ${successful} successful, ${failed} failed`);
